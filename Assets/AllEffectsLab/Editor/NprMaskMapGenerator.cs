...
public static class NprMaskMapGenerator
{
    ...
    [MenuItem("Tools/NPR 风味/生成三风味 Mask(FaceShadow/OutlineWidth)")]
    public static void GenerateAllMasks()
    {
        ...
        Debug.Log("[NPR][Mask] 三风味 Mask 生成完成。");
    }

    private static void GenerateRole(string role)
    {
        Texture2D srcFace = LoadReadable($"{SrcRoot}/T_{role}_FaceShadow.png");
        Texture2D srcOutline = LoadReadable($"{SrcRoot}/T_{role}_OutlineWidth.png");
        Texture2D srcLight = LoadReadable($"{SrcRoot}/T_{role}_LightMap.png");
    ...
    private static void GenerateStyle(string role, string style, Texture2D srcFace, Texture2D srcOutline, Texture2D srcLight, float faceContrast, float outlineContrast, float edgeBoost)
    {
        ...
        SaveGray($"{DstRoot}/{style}/T_{role}_FaceShadow.png", w, h, outFace);
        SaveGray($"{DstRoot}/{style}/T_{role}_OutlineWidth.png", w, h, outOutline);
    }
