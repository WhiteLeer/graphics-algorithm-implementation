...
{
    private const string ProfileRoot = "Assets/MaterialFX/NPR/NPR-Core/Materials/Profiles";
    private const string LockedRoot = "Assets/MaterialFX/NPR/NPR-Core/Materials/LockedProfiles";
    ...
    [MenuItem("Tools/NPR 风味/锁定当前三风味参数(保存快照)")]
    public static void SaveLockedProfiles()
    {
        EnsureFolder("Assets/MaterialFX/NPR/NPR-Core/Materials", "LockedProfiles");

        var md = new StringBuilder();
        md.AppendLine("# NPR-3 Locked Parameters");
        md.AppendLine();
        md.AppendLine("- Generated At: " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
        md.AppendLine("- Source: `Assets/MaterialFX/NPR/NPR-Core/Materials/Profiles`");
        ...
        {
            string srcDir = $"{ProfileRoot}/{style}/CharacterConverted";
            string dstStyle = $"{LockedRoot}/{style}";
            EnsureFolder(LockedRoot, style);
            EnsureFolder(dstStyle, "CharacterConverted");
            string dstDir = $"{dstStyle}/CharacterConverted";
    ...
    [MenuItem("Tools/NPR 风味/恢复锁定参数(覆盖当前)")]
    public static void RestoreLockedProfiles()
    {
        int restored = 0;
        foreach (string style in Styles)
        {
            string srcDir = $"{LockedRoot}/{style}/CharacterConverted";
            string dstDir = $"{ProfileRoot}/{style}/CharacterConverted";
            string[] guids = AssetDatabase.FindAssets("t:Material", new[] { srcDir });
            foreach (string guid in guids)
            {
                string srcPath = AssetDatabase.GUIDToAssetPath(guid);
