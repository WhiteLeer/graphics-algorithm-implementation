...
public class NprStyleSwitcherWindow : EditorWindow
{
    private const string BaseMaterialRoot = "Assets/MaterialFX/NPR/NPR-Core/Materials/BaseCharacter";
    private const string ProfileMaterialRoot = "Assets/MaterialFX/NPR/NPR-Core/Materials/StyleProfiles";
    private const string ProfileTextureRoot = "Assets/MaterialFX/NPR/NPR-Core/Textures/StyleMaps";
    private const string GeneratedTextureRoot = "Assets/MaterialFX/NPR/NPR-Core/Textures/SourceMaps";
    ...
    [MenuItem("Tools/NPR 风格切换器")]
    public static void Open()
    {
        GetWindow<NprStyleSwitcherWindow>("NPR 风格切换");
    }

    [MenuItem("Tools/NPR 风格/切换为原神(选中)")]
    public static void ApplyGenshinSelection()
    {
        SwitchStyle(Styles[0], true);
    }

    [MenuItem("Tools/NPR 风格/切换为崩铁(选中)")]
    public static void ApplyHsrSelection()
    {
        SwitchStyle(Styles[1], true);
    }

    [MenuItem("Tools/NPR 风格/切换为绝区零(选中)")]
    public static void ApplyZzzSelection()
    {
        SwitchStyle(Styles[2], true);
    }

    [MenuItem("Tools/NPR 风格/切换为原神(全场景)")]
    public static void ApplyGenshinAll()
    {
        SwitchStyle(Styles[0], false);
    }

    [MenuItem("Tools/NPR 风格/切换为崩铁(全场景)")]
    public static void ApplyHsrAll()
    {
        SwitchStyle(Styles[1], false);
    }

    [MenuItem("Tools/NPR 风格/切换为绝区零(全场景)")]
    public static void ApplyZzzAll()
    {
        SwitchStyle(Styles[2], false);
    }

    private void OnGUI()
    {
        EditorGUILayout.LabelField("整体渲染风格", EditorStyles.boldLabel);
        _styleIndex = EditorGUILayout.Popup("目标风格", _styleIndex, new[] { "原神", "崩铁", "绝区零" });
        _applyToSelection = EditorGUILayout.Toggle("仅作用于当前选中根节点", _applyToSelection);

        EditorGUILayout.Space(8);
        if (GUILayout.Button("一键构建并切换"))
        {
            SwitchStyle(Styles[_styleIndex], _applyToSelection);
        }

        EditorGUILayout.HelpBox("会自动构建该风格独立材质/贴图并替换 Renderer 材质。\n风格是独立 Shader + 独立材质目录。", MessageType.Info);
    ...
    private static void SwitchStyle(StyleDef style, bool onlySelection)
    {
        EnsureFolder("Assets/MaterialFX/NPR/NPR-Core/Materials", "StyleProfiles");
        EnsureFolder(ProfileMaterialRoot, style.Key);
        EnsureFolder(ProfileMaterialRoot + "/" + style.Key, "Character");
        EnsureFolder("Assets/MaterialFX/NPR/NPR-Core/Textures", "StyleMaps");
        EnsureFolder(ProfileTextureRoot, style.Key);

        Shader shader = AssetDatabase.LoadAssetAtPath<Shader>(style.ShaderPath);
        if (shader == null)
        {
            Debug.LogError("找不到风格 Shader: " + style.ShaderPath);
            return;
        }
        ...
        Debug.Log($"[NPR] 已切换风格到 {style.DisplayName}，渲染器 {rendererCount} 个，材质槽 {slotCount} 个。");
    }
    ...
    private static void BuildStyleTextures(string styleKey)
    {
        ...
        foreach (string role in roles)
        {
            ...
            {
                string src = GeneratedTextureRoot + $"/{TexturePrefix}_{role}_{kind}.png";
                string dst = styleDir + $"/{TexturePrefix}_{role}_{kind}.png";
    ...
    {
        string styleMatRoot = ProfileMaterialRoot + "/" + style.Key + "/Character";
    ...
    private static void ApplyStyleTexturesToMaterial(Material mat, string styleKey, MaterialRole role)
    {
        ...
        Texture2D light = AssetDatabase.LoadAssetAtPath<Texture2D>($"{ProfileTextureRoot}/{styleKey}/{TexturePrefix}_{roleName}_LightMap.png");
        Texture2D face = AssetDatabase.LoadAssetAtPath<Texture2D>($"{ProfileTextureRoot}/{styleKey}/{TexturePrefix}_{roleName}_FaceShadow.png");
        Texture2D outline = AssetDatabase.LoadAssetAtPath<Texture2D>($"{ProfileTextureRoot}/{styleKey}/{TexturePrefix}_{roleName}_OutlineWidth.png");
    ...
    private static Dictionary<string, Material> LoadStyleMap(string styleKey)
    {
        ...
        string root = ProfileMaterialRoot + "/" + styleKey + "/Character";
    ...
    private static MaterialRole ClassifyRole(Material mat)
    {
        ...
        if (name.Contains("hair") || matName.Contains("hair"))
        {
        ...
        // Safe fallback for generic material names: treat as body to avoid face-map pollution.
        return MaterialRole.Body;
    }
